...
CSDE1TMAreferenced_H_score <- CSDE1TMAreferenced_H_score %>% filter(Num.Detections > 1000)
...
CSDE1TMAreferenced_H_score$H.score <- as.numeric(CSDE1TMAreferenced_H_score$H.score)
CSDE1scoresbyhistNum <- group_by(CSDE1TMAreferenced_H_score, Histopathology.Reference.Number) %>% summarize(m = mean(H.score))
colnames(CSDE1scoresbyhistNum)  <- c("HistopathologyNumber", "CSDE1Hscore")

#Merge the data
df_list <- list(CD3countsbyhistNum, CD8countsbyhistNum ,CD4countsbyhistNum ,FOXP3countsbyhistNum, HLA_ABCscoresbyhistNum, HLA_ABCHscoresbyhistNum, HLA_C2scoresbyhistNum, HLA_C2HscoresbyhistNum, HLA_EscoresbyhistNum, HLA_EHscoresbyhistNum, TAP1scoresbyhistNum, TAP1HscoresbyhistNum, CSDE1scoresbyhistNum)
...
cor <- Hmisc::rcorr(omitNAAllmarkerMerged %>% as.matrix())
nm = rownames(cor$r)
m = t(combn(nm, 2))
...
HLA_C2percentagescore.surv_count.cut <- surv_cutpoint(
  ...
  time = "OS.months",
  event = "Survival.status",
  variables = "HLA_C2percentagescore", minprop = 0.2, maxprop = 0.8
)
...
# palette = "npg" (nature publishing group), see ?ggpubr::ggpar
plot(HLA_C2percentagescore.surv_count.cut, "HLA_C2percentagescore", palette = "npg", bins = 166)
...
library(survival)
library(RTCGA)
fit <- survfit(Surv(OS.months, Survival.status) ~ HLA_C2percentagescore,
...
HLA_Epercentagescore.surv_count.cut <- surv_cutpoint(
  ...
  time = "OS.months",
  event = "Survival.status",
  variables = "HLA_Epercentagescore", minprop = 0.45,maxprop = 0.55
)
...
# palette = "npg" (nature publishing group), see ?ggpubr::ggpar
plot(HLA_Epercentagescore.surv_count.cut, "HLA_Epercentagescore", palette = "npg", bins = 166)
...
library(survival)
library(RTCGA)
fit <- survfit(Surv(OS.months, Survival.status) ~ HLA_Epercentagescore,
...
TAP1percentagescore.surv_count.cut <- surv_cutpoint(
  ...
  time = "OS.months",
  event = "Survival.status",
  variables = "TAP1percentagescore", minprop = 0.2, maxprop = 0.7
)
...
# palette = "npg" (nature publishing group), see ?ggpubr::ggpar
plot(TAP1percentagescore.surv_count.cut, "TAP1percentagescore", palette = "npg", bins = 166)
...
library(survival)
library(RTCGA)
fit <- survfit(Surv(OS.months, Survival.status) ~ TAP1percentagescore,
...
TMACLINmergeHLA_C2percentagescore <- TMACLINmerge[,c("Histopathology.Reference.Number","OS.months", "Survival.status", "HLA_C2percentagescore")]
...
TMACLINmergeHLA_Epercentagescore <- TMACLINmerge[,c("Histopathology.Reference.Number","OS.months", "Survival.status", "HLA_Epercentagescore")]
...
TMACLINmergeTAP1percentagescore <- TMACLINmerge[,c("Histopathology.Reference.Number","OS.months", "Survival.status", "TAP1percentagescore")]
...
covariates <- c("CD3countpermm2",
                ...
                "CD8countpermm2", 
                "FOXP3countpermm2", 
                "HLA_ABCpercentagescore",
                "HLA_ABCHscore",
                "HLA_Epercentagescore",
                "HLA_EHscore",
                "HLA_C2percentagescore",
                "HLA_C2Hscore",
                "TAP1percentagescore",
                "TAP1Hscore")
...
univ_models <- lapply(univ_formulas, function(x){coxph(x, data = COXphoptimalcutoffs)})
...
forest_model(model_list = univ_models,
             ...
             recalculate_width = 3)
library(grid)
grid.text("Univariate OS", .3, 0.97, gp=gpar(cex=3, fontsize = 6), check.overlap = T)
...
#simplify values
COXphoptimalcutoffsClinMerge$pT[COXphoptimalcutoffsClinMerge$pT %in% c("1", "1a", "1b") ] <- "T0-1"

COXphoptimalcutoffsClinMerge$pT[COXphoptimalcutoffsClinMerge$pT %in% c("2", "2", "3", "4", "4a") ] <- "T2-4"
...
COXphoptimalcutoffsClinMerge$pN[COXphoptimalcutoffsClinMerge$pN %in% c("1", "2", "3") ] <- "N1-3"
COXphoptimalcutoffsClinMerge$pN[COXphoptimalcutoffsClinMerge$pN %in% "0" ] <- "N0"
...
COXphoptimalcutoffsClinMerge$CD4countpermm2
explanatory   <- c(    "Age",
                       "Sex",
                       "pT",
                       "pN",
